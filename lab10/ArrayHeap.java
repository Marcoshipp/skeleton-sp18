...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void swim(int index) {
        ...
        if (index > 1 && getNode(parentIndex(index)).priority() > getNode(index).priority()) {
    ...
    private void sink(int index) {
        ...
        validateSinkSwimArg(index);
        if (getNode(leftIndex(index)) == null && getNode(leftIndex(index)) == null) {
            return;
        }
        int yield = min(leftIndex(index), rightIndex(index));
        if (getNode(index).priority() > getNode(yield).priority()) {
            swap(yield, index);
            sink(yield);
        }
        return;
    ...
    public void changePriority(T item, double priority) {
        for (int i = 1; i < size(); i++) {
            if (contents[i].item().equals(item)) {
                double originalPriority = contents[i].priority();
                contents[i].myPriority = priority;
                if (originalPriority > priority) {
                    swim(i);
                } else if (originalPriority < priority) {
                    sink(i);
                }
                break;
            }
